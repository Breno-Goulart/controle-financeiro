...
import { 
    ...
    GoogleAuthProvider,
    signInWithPopup,
    signInAnonymously // Usar para login anónimo se não houver token customizado
...
// ##############################################################################
// # IMPORTANTE: COLOQUE AS SUAS CREDENCIAIS REAIS DO FIREBASE AQUI!          #
// # Copie o objeto de configuração COMPLETO diretamente da sua consola Firebase.#
// # Exemplo: { apiKey: "SUA_CHAVE", authDomain: "SEU_DOMINIO", ... }           #
// # AVISO: Hardcodificar chaves de API em repositórios públicos não é a        #
// # prática mais segura para aplicações em produção. Para maior segurança,    #
// # considere usar funções serverless (ex: Netlify Functions) para servir estas#
// # chaves, ou um processo de build que injete variáveis de ambiente.         #
// ##############################################################################
const firebaseConfig = {
    apiKey: "SEU_API_KEY_AQUI", // <-- SUBSTITUA PELO SEU API KEY
    authDomain: "SEU_AUTH_DOMAIN_AQUI", // <-- SUBSTITUA PELO SEU AUTH DOMAIN
    projectId: "SEU_PROJECT_ID_AQUI", // <-- SUBSTITUA PELO SEU PROJECT ID
    storageBucket: "SEU_STORAGE_BUCKET_AQUI", // <-- SUBSTITUA PELO SEU STORAGE BUCKET
    messagingSenderId: "SEU_MESSAGING_SENDER_ID_AQUI", // <-- SUBSTITUA PELO SEU MESSAGING SENDER ID
    appId: "SEU_APP_ID_AQUI" // <-- SUBSTITUA PELO SEU APP ID
};

// Para o Netlify, o appId é simplesmente o projectId da sua configuração.
// Removemos a dependência de `__app_id` e `__initial_auth_token` que são específicos do ambiente Canvas.
...
const RECURRING_MONTHS_AHEAD = 11; // Para gerar 12 lançamentos (mês atual + 11 futuros)
...
async function categorizeDescription(description) {
    ...
    // A chave de API para a Gemini API não precisa ser exposta diretamente no cliente.
    // Em um ambiente de produção, isto seria tratado por uma função de backend ou proxy.
    // Para fins de demonstração em ambientes como Netlify, a chave seria injetada durante o build,
    // ou a chamada API seria feita através de uma Netlify Function para maior segurança.
    // Por enquanto, mantenha-a vazia ou use uma chave de teste de forma consciente.
    const apiKey = ""; 
...
function initializeUI() {
    ...
    if (diaInput) diaInput.addEventListener('change', () => {
        const selectedDate = new Date(diaInput.value + 'T00:00:00');
        if (!isNaN(selectedDate.getTime())) {
            if (mesInput) mesInput.value = selectedDate.getMonth() + 1;
            if (anoInput) anoInput.value = selectedDate.getFullYear();
    ...
    if (isRecurringCheckbox) isRecurringCheckbox.addEventListener('change', (event) => {
        if (event.target.checked) {
            if (parcelaAtualSelect) parcelaAtualSelect.value = '0';
            if (totalParcelasSelect) totalParcelasSelect.value = '0';
        }
    ...
    if (setHouseholdIdBtn) setHouseholdIdBtn.addEventListener('click', () => {
        if (!joinHouseholdIdInput || !householdIdDisplay) {
            console.error("Elementos para definir Household ID não encontrados.");
            return;
        }
        ...
        if (newHouseholdId) {
            ...
            householdIdDisplay.textContent = `ID da Família/Casa: ${currentHouseholdId}`;
    ...
    try {
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                try {
                    // Tenta login anónimo se nenhum utilizador estiver logado
            ...
            if (householdIdDisplay) householdIdDisplay.textContent = `ID da Família/Casa: ${currentHouseholdId}`;
